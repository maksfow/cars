...
def edit_user_info_db(user_id, edit_info, new_info):
    ...
    if exact_user:
        ...
        if edit_info == 'sale':
           if exact_user.sale is None:
              exact_user.sale = new_info
           else:
               if new_info not in exact_user.sale:
                   exact_user.sale += ',' + new_info
        elif edit_info == 'purchase':
           if exact_user.purchase is None:
              exact_user.purchase = new_info
           else:
               if new_info not in exact_user.purchase:
                   exact_user.purchase += ',' + new_info
